...
from .self_attention import ImageSelfAttention
from ..SCAHN.weight_attention import WeightAttention
...
class ResNet(BasicModule):
    ...
    def __init__(self, bit, block, layers,
        ...
        self.s_att = ImageSelfAttention(bit)
    ...
    def forward(self, x: torch.Tensor):
        ...
        middle_hash = torch.cat(middle_hash)

        w = self.weight(*middle_hash)
        w = w.unsqueeze(2)
        y = self.s_att(w)
        y = y.squeeze()
